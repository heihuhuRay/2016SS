...
void CAN_MessageBufferInit(void)
{
    ...
    CAN_0.BUF[2].CS.B.LENGTH = 0x1; //1 byte
    ...
    CAN_0.BUF[11].CS.B.LENGTH = 0x1; //1 byte
    CAN_0.BUF[11].ID.B.STD_ID = 0x404;
    //Individual mask register
    CAN_0.RXIMR[].R = 0x01FFFFFF;
    ...
    CAN_0.BUF[12].CS.B.LENGTH = 0x1; //1 byte
...
void PITCHANNEL0() { //Timer0 interrupt = 200ms
    timer0Flag = 1; //Setting a Flag when the timer is 
    CANSEND0();
