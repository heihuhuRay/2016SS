...
void main(void)
{
	...
	LED7 = ~LED7; //Blinking LED7
      ...
      if(timer0Flag == 1) {
	CAN_Send_200_Interrupt();
	timer0Flag = 0;
      }

      if(timer1Flag == 1) {
	CAN_Send_400_Interrupt();
	timer1Flag = 0;
      }
    }
}
...
void CAN_MessageBufferInit(void)
{
    ...
    CAN_0.BUF[2].CS.B.LENGTH = 0x0; //1 byte
    ...
    CAN_0.BUF[11].CS.B.LENGTH = 0x0; //1 byte
    ...
    CAN_0.BUF[12].CS.B.LENGTH = 0x0; //1 byte
...
void CAN_Send_200_Interrupt(void) {
    int i;
...
void CAN_Send_400_Interrupt(void) {
    int i;
    
    //Sending Door Status (M_ID = 0x403) through Buffer 10
    CAN_0.BUF[10].DATA.B[0] = (rightDoorStatus << 2) | (leftDoorStatus << 1) | (openedDoorsWarning);
    for(i = 1; i < 8; i++)
...
void CANRCV0(void) {
    ...
    speedValue = CAN_0.BUF[0].DATA.B[0] + (CAN_0.BUF[0].DATA.B[1] << 8); //Value = LSB + (MSB << 8)
    CAN_0.BUF[0].CS.B.CODE = 0x4; //0b0100
    ...
    ECU4_aliveSignal = CAN_0.BUF[2].DATA.B[0]; //Receiving ECU4 Alive Signal
    CAN_0.BUF[1].CS.B.CODE = 0x4; //0b0100
...
void PITCHANNEL0() { //Timer0 interrupt = 200ms
    timer0Flag = 1; //Setting a Flag when the timer is 200ms
...
void PITCHANNEL1() { //Timer1 interrupt = 400ms
    timer1Flag = 1; //Setting a Flag when the timer is 400ms
